...
import RecommendationCard2 from '../components/RecommendationCard2';
...
const RecipeDetails2 = () => {
  ...
  const { strMeal, strDrink, strMealThumb, strDrinkThumb, strCategory: category,
    strInstructions: instructions, strYoutube: youtubeUrl, strAlcoholic: alcoholicOrNot,
    strArea: nationality } = recipeData;
  ...
  const subcategory = isMeal ? category : alcoholicOrNot;
  ...
  useEffect(() => {
    const isAlreadyInProgressOnLoad = () => {
      ...
      const isAlreadyInProgress = localStorageData.some((item) => item.includes(id));
      // const idList = isMeal
  ...
  const onFavoriteClick = () => {
    const payload = { id, type, nationality, category, alcoholicOrNot, name, image };
    if (!isFavorite) {
